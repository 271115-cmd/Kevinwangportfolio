...
function pavilion(B, { w, bodyH, eaves, roofH, finialR, tile = 'yellow', terraceMat = 'marble' }) {
  const t = terrace(B, { w: w * 1.35, d: w * 1.35, tiers: 1, tierH: w * 0.12, inset: 1, stair: false, mat: terraceMat });
  const b = bayBody(B, { w, d: w, h: bodyH, frontBays: 3, depthBays: 3, y: t.topY, dougong: true });
  roof(B, { w: w * 1.2, d: w * 1.2, y: b.topY, eaves, roofH, ridgeFrac: 0, tile });
...
const builders = {
  ...
  xiannongtan(B) {
    const t = terrace(B, { w: 53, d: 28, tiers: 1, tierH: 1.6, inset: 2, balustrade: false, stair: true, mat: 'stone' });
    const b = bayBody(B, { w: 47, d: 23, h: 11, frontBays: 7, depthBays: 3, y: t.topY });
    const r = roof(B, { w: 53, d: 29, y: b.topY, eaves: 1, roofH: 9, gable: false, tile: 'black' });
    ridgeOrnaments(B, { ridgeHalf: r.ridgeHalf, y: r.ridgeY - 0.4, s: 1.5, tile: 'black' });
  },
  ...
  'south-road'(B) {
    B.box('earth', 18, 1.0, 46, 0, -0.5, 0);                 // sunken pit base (rammed earth)
    B.box('stone', 4.8, 0.4, 44, 0, 0.2, 0);                 // central granite imperial way
    B.box('earth', 5.6, 0.18, 44, 6.2, 0.09, 0);             // east earthen rut path
    B.box('earth', 5.6, 0.18, 44, -6.2, 0.09, 0);            // west earthen rut path
  ...
  'tiananmen-square'(B) {
    ...
    roof(B, { w: 9, d: 6.5, y: sh.topY, eaves: 1, roofH: 2.8, gable: true, tile: 'stone' });  // carved granite cap, not glaze
  },
  ...
  tiananmen(B) {
    const base = gateBase(B, { w: 66, h: 14, d: 34, passages: 5, passW: 4.6, parapet: false, mat: 'wall' });
    ...
    roof(B, { w: 60, d: 28, y: b.topY, eaves: 2, roofH: 8, gable: true, tile: 'yellow' });
  },
  ...
  duanmen(B) {
    const base = gateBase(B, { w: 60, h: 14, d: 30, passages: 5, passW: 4.2, parapet: false, mat: 'wall' });
    ...
    roof(B, { w: 54, d: 26, y: b.topY, eaves: 2, roofH: 7, gable: true, tile: 'yellow' });
  },
  ...
  'forbidden-city'(B) {
    ...
    const r = roof(B, { w: 66, d: 38, y: b.topY, eaves: 2, roofH: 10, gable: false, tile: 'yellow' });
    ridgeOrnaments(B, { ridgeHalf: r.ridgeHalf, y: r.ridgeY - 0.5, s: 2.0, tile: 'yellow' });
  },
  ...
  taimiao(B) {
    ...
    const r = roof(B, { w: 72, d: 40, y: b.topY, eaves: 2, roofH: 9, gable: false, tile: 'yellow' });
    ridgeOrnaments(B, { ridgeHalf: r.ridgeHalf, y: r.ridgeY - 0.5, s: 1.7, tile: 'yellow' });
  },
  ...
  shejitan(B) {
    ...
    for (const [x, z, ax] of [[0, 10.5, 0], [0, -10.5, 0], [10.5, 0, 1], [-10.5, 0, 1]])
      B.box('marble', ax ? 2.4 : 5, t.topY, ax ? 5 : 2.4, x, t.topY / 2, z);   // four marble stairs
    B.box('stone', 1.1, 2.0, 1.1, 0, t.topY + 1.0, 0);                          // 社主石 centre marker (dark stone)
    // low precinct wall (waist-high), set well back — grey brick
    const S = 24;
    for (const [w, d, x, z] of [[S * 2, 1, 0, S], [S * 2, 1, 0, -S], [1, S * 2, S, 0], [1, S * 2, -S, 0]])
      B.box('brick', w, 2.4, d, x, 1.2, z);
  ...
  jingshan(B, M) {
    ...
    // rank-graded 5-colour crown: yellow centre → green pair → blue pair (五方)
    place(0, { w: 12, bodyH: 6, eaves: 3, roofH: 9, finialR: 0.9, tile: 'yellow', terraceMat: 'marble' });  // Wanchun (centre)
    place(24, { w: 9, bodyH: 4.5, eaves: 2, roofH: 6.5, finialR: 0.7, tile: 'green', terraceMat: 'stone' });
    place(-24, { w: 9, bodyH: 4.5, eaves: 2, roofH: 6.5, finialR: 0.7, tile: 'green', terraceMat: 'stone' });
    place(41, { w: 7.5, bodyH: 3.6, eaves: 2, roofH: 5, finialR: 0.6, tile: 'blue', terraceMat: 'stone' });
    place(-41, { w: 7.5, bodyH: 3.6, eaves: 2, roofH: 5, finialR: 0.6, tile: 'blue', terraceMat: 'stone' });
  },
  ...
  wanning(B) {
    archBridge(B, { span: 16, width: 17, rise: 3, count: 1, deckW: 17, mat: 'stone' });
  ...
  'bell-drum'(B, M) {
    ...
    roof(bell, { w: 28, d: 28, y: bb.topY, eaves: 2, roofH: 12, gable: true, tile: 'black' });
